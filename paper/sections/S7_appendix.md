...
As a simulator-free proxy we implemented bench_policy_proxy.py, which computes region metrics on the gripper (label ids 29-35) and the apple (label id 44): AbsRel, PSNR, and the 3D centroid error in cm between predicted and ground-truth pointmaps inside the same mask, per predicted frame 1 to 10. This proxy has not been run and we report no numbers for it.
